...
class HomeCellIsland: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        collectionView.register(UINib(nibName: "CellA_One", bundle: nil), forCellWithReuseIdentifier: "islandAnimal")
...
extension HomeCellIsland: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return sharedData.st.villagersInMyIsland.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "islandAnimal", for: indexPath) as? CellA_One else{fatalError()}
        
//        cell.imageView.layer.cornerRadius = cell.imageView.frame.height / 4
        cell.imageView.layer.cornerRadius = layoutWidth / 2
        
        cell.nameLabel.text = sharedData.st.nameInMyIsland[indexPath.item]
        //        cell.imageView.image = UIImage(data: sharedData.st.iconInMyIsland[indexPath.item])
        DispatchQueue.main.async {
            guard let img = UIImage(data: sharedData.st.iconInMyIsland[indexPath.item])else{return}
...
extension HomeCellIsland: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        delegate?.didTapVillagerCell(sharedData.st.villagersInMyIsland[indexPath.item])
    }
}

extension HomeCellIsland {  //CollectionView Layout
